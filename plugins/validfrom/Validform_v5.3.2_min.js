...
(function(d, f, b) {
    ...
    a.defaults = {tiptype: 3, tipSweep: false, showAllError: true, postonce: false, ajaxPost: false};
    ...
    if ($.Datatype) {
        ...
        $.extend($.Datatype, {
            "price": function(gets, obj, curform, regxp) { //价格的验证
                ...
                //var re = /^-?((0.[\d]{1,2})|([1-9]\d{0,5}(\.[\d]{1,2})?))$/;    //为了能让旅付宝测试通过，不限制发线时的价格必须大于1元
                if (re.test(gets) || gets == "0")
                    return true;
                return false;
            },
            /*
            ...
            "ge": function(gets, obj, curform, datatype) {  //大于等于
                var withele = $.trim(obj.attr("with"));
                var val = curform.find(withele + ",[name='" + withele + "']").val() || 0;

                if (gets < $.trim(val)) {
                    return false;
                }
            },
            "le": function(gets, obj, curform, datatype) {  //小于等于
                var withele = $.trim(obj.attr("with"));
                var val = curform.find(withele + ",[name='" + withele + "']").val() || 0;
                if (parseInt(gets) > parseInt($.trim(val))) {
                    return false;
                }
            },
            ...
            "tel": function(gets, obj, curform, datatype) {
                var gs = gets.split('|');
            ...
            "datecompare": function(gets, obj, curform, datatype) {
                var withele = $.trim(obj.attr("with"));
                var flag = $.trim(obj.attr('flag'));
                var val = curform.find(withele + ",[name='" + withele + "']").val() || withele;
                var sdate = val.match(/(\d{4})-(\d{1,2})-(\d{1,2})\s+(\d{1,2}):(\d{1,2})/);
                var sd = new Date(sdate[1], sdate[2], sdate[3], sdate[4], sdate[5]);
                var edate = gets.match(/(\d{4})-(\d{1,2})-(\d{1,2})\s+(\d{1,2}):(\d{1,2})/);
                var ed = new Date(edate[1], edate[2], edate[3], edate[4], edate[5]);
                if (flag == 'gt') { //大于
                    if (parseInt(sd.getTime()) > parseInt(ed.getTime())) {
                        return false;
                    }
                } else if (flag == 'lt') {
                    if (parseInt(sd.getTime()) < parseInt(ed.getTime())) {
